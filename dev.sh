...
workon djavue  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development djavue npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop djavue
    docker rm djavue
    docker run --name djavue -d --env-file /home/ubuntu/djavue.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/djavue:/dkdata \
        djavue start_web.sh
}
...
echo_green "Welcome to djavue's dev env"
